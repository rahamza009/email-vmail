  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="font-barlow text-2xl font-black flex-1" style={{ color: "#2D3A28" }}>New Blog Post</h1>
        <button onClick={() => save("draft")} disabled={saving}
          className="font-barlow text-sm font-bold px-4 py-2 rounded-xl border-2 transition-colors hover:border-[#2D3A28] disabled:opacity-50"
          style={{ borderColor: "rgba(45,58,40,0.2)", color: "#2D3A28" }}>
          Save Draft
        </button>
        <button onClick={() => save("published")} disabled={saving}
          className="font-barlow text-sm font-bold px-5 py-2 rounded-xl transition-opacity hover:opacity-90 disabled:opacity-50"
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="bg-white rounded-2xl border p-6 space-y-4" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
            <Field label="Title *">
              <input value={form.title} onChange={e => set("title", e.target.value)} placeholder="Your blog post title…"
            ...
            <Field label="Excerpt">
              <TextArea value={form.excerpt} onChange={v => set("excerpt", v)} rows={2} placeholder="Short description shown in blog listing…" />
            </Field>
          </div>
          ...
          {tab === "seo" && (
            <div className="bg-white rounded-2xl border p-6 space-y-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
              <Field label="Focus Keywords (comma-separated)">
                <TextInput value={form.keywords} onChange={v => set("keywords", v)} placeholder="email marketing firearms, FFL email, gun store klaviyo…" />
              </Field>
              <Field label="SEO Title">
                <TextInput value={form.seoTitle} onChange={v => set("seoTitle", v)} />
                <p className="font-inter text-xs mt-1" style={{ color: form.seoTitle.length > 60 ? "#e11d48" : "rgba(45,58,40,0.4)" }}>{form.seoTitle.length}/60 characters</p>
              </Field>
              <Field label="Meta Description">
                <TextArea value={form.seoDescription} onChange={v => set("seoDescription", v)} rows={3} />
                <p className="font-inter text-xs mt-1" style={{ color: form.seoDescription.length > 160 ? "#e11d48" : "rgba(45,58,40,0.4)" }}>{form.seoDescription.length}/160 characters</p>
              </Field>
              ...
              <div className="p-4 rounded-xl border" style={{ borderColor: "rgba(45,58,40,0.1)", backgroundColor: "#F9FAFB" }}>
                ...
                <p className="font-inter text-sm leading-snug" style={{ color: "#4d5156" }}>{form.seoDescription || form.excerpt || "Post description will appear here…"}</p>
              </div>
            </div>
          )}
          ...
          {tab === "og" && (
            <div className="bg-white rounded-2xl border p-6 space-y-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
              <Field label="OG Title">
                <TextInput value={form.ogTitle} onChange={v => set("ogTitle", v)} placeholder="Title shown when shared on social…" />
              </Field>
              <Field label="OG Description">
                <TextArea value={form.ogDescription} onChange={v => set("ogDescription", v)} rows={3} placeholder="Description shown when shared…" />
              </Field>
              <Field label="OG Type">
              ...
              <div className="rounded-xl overflow-hidden border" style={{ borderColor: "rgba(45,58,40,0.15)" }}>
                {form.coverImage && <img src={form.coverImage} alt="" className="w-full h-40 object-cover" />}
                {!form.coverImage && <div className="w-full h-40 flex items-center justify-center" style={{ backgroundColor: "#2D3A28" }}><span className="font-rajdhani font-bold tracking-widest uppercase" style={{ color: "#F5C124" }}>Email-Vmail</span></div>}
        ...
        <div className="space-y-5">
          ...
          <div className="bg-white rounded-2xl border p-5 space-y-3" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
            ...
            <button onClick={() => save("published")} disabled={saving}
              className="w-full font-barlow font-bold py-3 rounded-xl text-sm tracking-wide hover:opacity-90 disabled:opacity-50"
              style={{ backgroundColor: "#2D3A28", color: "#F5C124" }}>
              {saving ? "Saving…" : "Publish Now"}
            </button>
            <button onClick={() => save("draft")} disabled={saving}
              className="w-full font-barlow font-bold py-3 rounded-xl text-sm tracking-wide border-2 hover:border-[#2D3A28] disabled:opacity-50"
              style={{ borderColor: "rgba(45,58,40,0.2)", color: "#2D3A28" }}>
              Save as Draft
            </button>
          </div>
          ...
          <div className="bg-white rounded-2xl border p-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
            ...
            <button type="button" onClick={() => set("isActive", !form.isActive)}
              ...
              <div className="text-left">
                ...
                <p className="font-inter text-xs mt-0.5" style={{ color: "rgba(45,58,40,0.5)" }}>{form.isActive ? "Will appear on /blog" : "Will not appear on /blog"}</p>
              </div>
