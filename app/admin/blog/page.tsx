...
interface Post {
  ...
  createdAt: string; displayOrder: number; tags: string[];
}
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="font-barlow text-2xl font-black" style={{ color: "#2D3A28" }}>Blog</h1>
          <p className="font-inter text-sm mt-1" style={{ color: "rgba(45,58,40,0.5)" }}>
            {posts.length} posts &middot; {published} published &middot; {active} visible on site
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border-2 p-16 text-center bg-white" style={{ borderColor: "rgba(245,193,36,0.3)" }}>
          <p className="font-barlow text-xl font-bold mb-2" style={{ color: "rgba(45,58,40,0.4)" }}>No posts yet</p>
          <Link href="/admin/blog/new" className="font-barlow text-sm font-bold px-5 py-2.5 rounded-xl inline-block mt-4" style={{ backgroundColor: "#2D3A28", color: "#F5C124" }}>
            Write First Post
          </Link>
        </div>
      ) : (
        <div className="bg-white rounded-2xl border overflow-hidden" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
          ...
          <div className="hidden sm:grid grid-cols-12 gap-4 px-6 py-3 border-b" style={{ borderColor: "rgba(45,58,40,0.08)", backgroundColor: "#F9FAFB" }}>
            ...
            <span className="col-span-2 font-barlow text-xs font-bold tracking-widest uppercase" style={{ color: "rgba(45,58,40,0.45)" }}>Category</span>
            <span className="col-span-2 font-barlow text-xs font-bold tracking-widest uppercase" style={{ color: "rgba(45,58,40,0.45)" }}>Status</span>
            <span className="col-span-2 font-barlow text-xs font-bold tracking-widets uppercase" style={{ color: "rgba(45,58,40,0.45)" }}>Visible on Site</span>
          ...
          {visible.map((post, idx) => (
            <div key={post._id}
              ...
              <div className="sm:col-span-4">
                ...
                <p className="font-inter text-xs mt-0.5" style={{ color: "rgba(45,58,40,0.45)" }}>/blog/{post.slug}</p>
              </div>

              {/* Category */}
              <span className="sm:col-span-2 font-inter text-xs" style={{ color: "rgba(45,58,40,0.6)" }}>{post.category}</span>
              ...
              <div className="sm:col-span-2 flex items-center">
                <button
                  onClick={() => patch(post._id, { isActive: !post.isActive })}
                  className="relative inline-flex items-center gap-2 font-barlow text-xs font-bold"
                  title={post.isActive ? "Showing on website — click to hide" : "Hidden from website — click to show"}
                >
                  {/* Toggle pill */}
              ...
              {/* Order */}
              <span className="sm:col-span-1 font-barlow text-xs font-bold text-center" style={{ color: "rgba(45,58,40,0.4)" }}>
                #{post.displayOrder}
              </span>

              {/* Actions */}
              <div className="sm:col-span-1 flex gap-1 justify-start sm:justify-end">
