...
export default function EditCaseStudy() {
  ...
  const [form, setForm] = useState<BlogFormData>({ ...EMPTY_FORM, category: "Case Study", tags: ["case study"] });
  ...
  useEffect(() => {
    fetch("/api/admin/blog")
      ...
      .then(d => {
        ...
        const caseStudies = allPosts.filter((p: { category: string; tags: string[] }) =>
          p.category === "Case Study" || (Array.isArray(p.tags) && p.tags.includes("case study"))
        );
        setTotalPosts(caseStudies.length);
        const idx = caseStudies.findIndex((p: { _id: string }) => p._id === id);
        setPostIndex(idx + 1);
        const post = allPosts.find((p: { _id: string }) => p._id === id);
        if (post) {
          setForm({
            title: post.title ?? "",
            slug: post.slug ?? "",
            category: post.category ?? "Case Study",
            tags: Array.isArray(post.tags) ? post.tags : ["case study"],
    ...
    <div className="p-6 md:p-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="font-barlow text-xl font-black truncate" style={{ color: "#2D3A28" }}>{form.title || "Edit Case Study"}</h1>
          <p className="font-inter text-xs mt-0.5" style={{ color: "rgba(45,58,40,0.45)" }}>
            Case Study {postIndex} of {totalPosts} &nbsp;·&nbsp; Order #{form.displayOrder} &nbsp;·&nbsp;
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="xl:col-span-2 space-y-5">
          ...
          {tab === "seo" && (
            <div className="bg-white rounded-2xl border p-6 space-y-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
              <Field label="Focus Keywords">
                <TextInput value={form.keywords} onChange={v => set("keywords", v)} placeholder="email marketing case study, firearms email results…" />
              </Field>
          ...
          {tab === "og" && (
            <div className="bg-white rounded-2xl border p-6 space-y-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
              <Field label="OG Title"><TextInput value={form.ogTitle} onChange={v => set("ogTitle", v)} /></Field>
              <Field label="OG Description"><TextArea value={form.ogDescription} onChange={v => set("ogDescription", v)} rows={3} /></Field>
              <Field label="OG Type">
            ...
            <div className="bg-white rounded-2xl border p-6 space-y-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
              <Field label="Display Order (lower = appears first)">
                <input type="number" value={form.displayOrder} onChange={e => set("displayOrder", Number(e.target.value))}
                  className="w-full font-inter text-sm px-4 py-3 rounded-xl border-2 outline-none focus:border-[#F5C124]"
                  style={{ borderColor: "rgba(45,58,40,0.15)", color: "#2D3A28" }} />
                <p className="font-inter text-xs mt-1" style={{ color: "rgba(45,58,40,0.4)" }}>Case Study {postIndex} of {totalPosts} total</p>
              </Field>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border p-5 space-y-3" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
            ...
            {form.status === "published" && (
              <a href={`/blog/${form.slug}`} target="_blank" rel="noopener noreferrer"
                className="w-full font-barlow font-bold py-2.5 rounded-xl text-sm flex items-center justify-center gap-2 hover:opacity-70"
          ...
          <div className="bg-white rounded-2xl border p-5" style={{ borderColor: "rgba(45,58,40,0.1)" }}>
            ...
            <button type="button" onClick={() => set("isActive", !form.isActive)}
              ...
              <div className="text-left">
                ...
                <p className="font-inter text-xs mt-0.5" style={{ color: "rgba(45,58,40,0.5)" }}>{form.isActive ? "Appears on site" : "Hidden from site"}</p>
              </div>
          ...
          <div className="rounded-xl p-4 border" style={{ backgroundColor: "rgba(45,58,40,0.03)", borderColor: "rgba(45,58,40,0.1)" }}>
            ...
            <p className="font-inter text-xs leading-relaxed" style={{ color: "rgba(45,58,40,0.55)" }}>
              Case Study <strong>{postIndex}</strong> of <strong>{totalPosts}</strong><br />
              Order: <strong>#{form.displayOrder}</strong><br />
